        ...
        if ($statusUpload == 0) {
            ...
            if (mysqli_num_rows($select) > 0) {
                ...
                if (move_uploaded_file($_FILES['gambar']['tmp_name'], $target_file)) {
                    $query = mysqli_query($conn, "INSERT INTO tb_daftar_menu (gambar, nama_menu, keterangan, kategori, harga, stok) values ('" . $kode_random . $_FILES['gambar']['name'] . "', '$nama_menu' , '$keterangan', '$kategori_menu', '$harga', '$stok')");
                    if ($query) {
